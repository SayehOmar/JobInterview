...
export function parseForestSurfaceHectares(
...
export function buildEffectiveSpeciesRows(
  ...
  const essenceRaw =
    (forest as any)?.ESSENCE ??
    (forest as any)?.essence ??
    (forest as any)?.essences ??
    (forest as any)?.Essence ??
    (forest as any)?.ESPECES ??
    (forest as any)?.ESPECE ??
    null;
  ...
  const tfv = String((forest as any)?.TFV ?? (forest as any)?.tfv ?? "").trim();
  if (tfv) {
    return [{ species: tfv, areaHectares: baseHa, percentage: 100 }];
  }

  const cat = String((forest as any)?.TFV_G11 ?? (forest as any)?.tfv_g11 ?? "").trim();
  if (cat) {
      ...
      coveragePct:
        aCov != null && Number.isFinite(aCov) && aCov > 0
          ...
          : polygonAreaHa > 0
            ...
            : 0,
      plotCount: aPlots != null && aPlots > 0 ? aPlots : (forest as any)?.ID || (forest as any)?.id ? 1 : 0,
    };
  }

  if (!forest || !((forest as any).ID ?? (forest as any).id)) {
    return {
      totalForestHa: 0,
      coveragePct: 0,
      plotCount: 0,
